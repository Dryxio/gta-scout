...
4. Import the bundled text pack following [shared annotations](docs/shared-annotations.md) before requesting new visual reviews. Preview the matches, then apply them and build semantic vectors. Source-matched shared reviews require no new rendering. Reference-only entries remain leads requiring local inspection. Search multiple formulations and related functional categories. Inspect coverage. Opaque names and incomplete annotations mean absence in the shortlist is not proof that a suitable asset does not exist.
...
Keep tests synthetic and game-free. Never commit output, local paths, credentials, native assets, or private annotation payloads. The explicitly sanitized, versioned text packs in `data/annotations/` are public; use the exporter and publishing checks in docs/shared-annotations.md. Benchmark reports should disclose sample construction, missed candidates, unsupported assets and costs, without redistributing game data. Do not claim CP completion from render counts alone.
